...
class VideoTranscriptionController extends Controller
{
    public function showTranscribeForm()
    {
        ...
        // $results = session('results', []); // Retrieve any results stored in session
        // $response = session('response', ''); // Retrieve any response message stored in session
    ...
    public function showSingleTranscription($id)
    {
        // Find a single transcription by its transcription_id
        $transcription = Transcription::where('transcription_id', $id)->first(); // Use first() to get a single instance

        // Check if transcription is found
        if (!$transcription) {
            return response()->json([
        ...
        ], 200); // Return 200 OK status with transcription data
    }
    ...
    public function updateTranscription($transcriptionId)
    {
        ...
        if ($transcription) {
            ...
            try {
                ...
                $keypointsPrompt = "Extract 5 key points with timestamps from the following video transcription. The first key point is 'intro' and the last is 'conclusion'. LIMIT THE RESPONSE to one word for each key point! Always return the response in JSON format like this:
{
    'key_points': [
        {
            'Keyword': '…keyword…',
